...
router.post(
  ...
  [
    body('text').isLength({ min: 1 }).trim(),
    body('taskType').isIn(['improve_clarity', 'make_professional', 'fix_grammar', 'create_summary']),
  ],
  async (req, res, next) => {
    try {
      const errors = validationResult(req)
      ...
      // Return mock improvement if OpenAI is not configured
      if (!client) {
        logger.info('OpenAI not configured, using mock AI assistance')
        ...
        return res.json({
          success: true,
          taskType,
          taskName: taskNames[taskType as keyof typeof taskNames],
