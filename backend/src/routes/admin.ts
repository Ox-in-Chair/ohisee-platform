...
import { sendReportUpdate } from '../services/emailService'

const router = Router()

router.use(authorize('admin', 'manager', 'compliance'))
...
router.patch('/reports/:id', async (req, res, next) => {
  try {
    ...
    logger.info(`Report ${req.params.id} updated by ${req.user!.email}`)
